...
"""Reward function for the MCQ task with Llama models.

Implements the PA-GRPO reward (Eq. 7, 9, 10 of the paper):
  - Preliminary reward: correctness + format + length.
  - Cross-permutation unique-mode consistency reward.

Used by `scripts/run_mcq_llama.sh` via verl's `custom_reward_function` config.
"""
...
# 日志文件路径 (相对于项目根目录)
LOG_PATH = os.path.join(_PROJECT_ROOT, "logs", "mcq_llama3_1_8b.log")

# TensorBoard 日志目录
TB_LOG_DIR = os.path.join(_PROJECT_ROOT, "logs", "tb_mcq_llama3_1_8b")
_writer = None
_global_step = 0
...
def map_to_canonical(ans, permutation):
    ...
    Given the model's predicted letter under a specific permutation, return the
    corresponding letter in the original ABCD ordering. This implements the
    label-to-semantic mapping m_τ described in Section 3.3.1 of the paper.

    Example with ``permutation="BCDA"``:
        - position A (index 0) was originally option B
        - position B (index 1) was originally option C
        - position C (index 2) was originally option D
        - position D (index 3) was originally option A
...
def safe_log(index, pid, cuda, txt):
    """Append a single log line to ``LOG_PATH``.

    Uses raw file append rather than the ``logging`` module to avoid contention
...
def compute_score(
    ...
    Combines (1) a per-sample preliminary reward and (2) a cross-permutation
    consistency reward, as described in Section 3.3 of the paper.

    Cross-permutation consistency is computed *column-wise* across the rollouts
    of different permutations sharing the same ``original_index``. With P = 5
    permutations and N = 8 rollouts each, the reward function pairs the t-th
    rollout from every permutation and applies the unique-mode agreement rule
    (Eq. 9, 10): if a single semantic answer is the strict majority, samples
    matching it get +``consistency_weight``, others get -``consistency_weight``.
    ...
    Args:
        ...
        extra_infos: List[dict], each entry must contain ``original_index`` and
            ``permutation`` fields populated by the data preprocessing pipeline.
